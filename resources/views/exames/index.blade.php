...
@section('content')
    <div x-data="setup()" x-init="fetchData()">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-2xl text-[#155CA9] ">Exames</h2>
            </div>
            <div>
                <a href="{{ route('exames.create') }}"
                    class="hover:cursor-pointer p-2 bg-blue-600 hover:bg-blue-800 text-white rounded-lg">Adicionar Exame</a>
            </div>
        </div>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 ">
                ...
                <tbody>
                    <tr x-show="exameDataSize == 0">
                        <td class="text-center" colspan="3">Nenhum Registro Encontrado</td>
                    </tr>

                    <template x-for="(exame, index) in examesData.data" :key="index">
                        <tr class="bg-white border-b hover:bg-gray-50 ">
                            <td x-text="exame.id" class="text-center py-4"></td>
                            <td x-text="exame.nome" class="text-center"></td>
                            <td class="">
                                <div class="flex gap-2">
                                    ...
                                    <a type="button" @click="excluirExame(exame.id)">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                    ...
                    <ul class="inline-flex -space-x-px text-sm">
                        <li>
                            <a href="#"
                                ...
                                @click="fetchData(examesData.current_page - 1)"
                        ...
                        <template x-for="(page, index) in examesData.links" :key="index">
                            <li>
                                <a href="#"
                                    x-bind:class="{
        ...
        function setup() {
            return {
                ...
                fetchData(page) {
                    ...
                    try {
                        const url = "{{ route('exames.listarExames') }}";
                        ...
                        response.then(response => {
                            this.examesData = response.data;
                            this.totalPages = response.data.last_page;

                            this.examesData.links.shift(); // Remove o link "Primeira" se existir
                            this.examesData.links.pop(); // Remove o link "Última" se existir

                            this.exameDataSize = this.examesData.data.length;
                ...
                excluirExame(id){
                    if(id == null || id == undefined){
                        Swal.fire({
                            icon: 'error',
                            title: 'Erro',
                            text: 'ID do exame não encontrado.',
                    ...
                    Swal.fire({
                        ...
                        if(result.isConfirmed){
                            ...
                            .then(response => {
                                ...
                                this.fetchData(); // Atualiza a lista de exames
                            })
                        }
                    })
